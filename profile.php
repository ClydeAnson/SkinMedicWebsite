...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['upload_picture'])) {
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($fileTmpPath, $filePath)) {
            ...
            $errorMsg = "⚠️ Failed to upload the picture. Please try again.";
        }
    } else {
        $errorMsg = "⚠️ Please select a valid image file.";
    }
}
...
// ✅ Fetch user info
$userQuery = $conn->prepare("SELECT firstName, lastName, email, phone, birthdate, gender, address, profile_picture FROM users WHERE user_id = ?");
